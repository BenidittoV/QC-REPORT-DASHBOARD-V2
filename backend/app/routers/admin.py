...
from app.services.file_service import create_file_record, remove_file_if_exists, save_upload_file, serialize_file
...
def list_files(
    ...
    files = db.scalars(
        select(ExcelFile).options(selectinload(ExcelFile.uploader)).order_by(ExcelFile.upload_date.desc())
    ).all()
